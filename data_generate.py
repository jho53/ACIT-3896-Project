...
for term in range(num_term):
    for course in range(num_course):
        for set1 in range(cit_num_set):
            temp_cit_crn = "CIT"+str(term+1)+str(course+1)+str(set1+1)
            temp_cap = 30
            temp_crn_cit = {
                "crn": temp_cit_crn,
            ...
            set_list["CIT" + str(term + 1) + str(set1 + 1)]["crns"].append(temp_cit_crn)

        for set2 in range(cst_num_set):
            temp_cst_crn = "CST"+str(term+1)+str(course+1)+str(set2+1)
            temp_cap = 30
            temp_crn_cst = {
                "crn": temp_cst_crn,
            ...
            set_list["CST" + str(term + 1) + str(set2 + 1)]["crns"].append(temp_cst_crn)
